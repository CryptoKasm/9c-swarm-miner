...
# Intro
Write-Host("-----------------------------------------")
Write-Host("Nine Chronicles - CryptoKasm Swarm Miner")
Write-Host("  Setup Windows 10 Home for Docker")
Write-Host("-----------------------------------------")
...
# Function: Check for Kernel Update
...
# Function: Get list of installed distros 
...
# Function: Check if distro exists
...
# Function: Download distro from store **NOT USED BUT SAVED FOR FUTURE**
...
# Function: Select distro to download
...
function Install-Distro ($distro) {
    ...
    Write-Host("> Install Distro " + $distro.Name + ".")
    if (Get-WSLExistance($distro)) {
        Write-Host("   --Found an existing distro: " + $distro.Name)
    } else {
...
function Add-Docker {
    $dockerDL="https://desktop.docker.com/win/stable/Docker%20Desktop%20Installer.exe"
    ...
    If(-Not $installed) {
        ...
        Invoke-WebRequest -Uri $dockerDL -OutFile $dockerInstaller -UseBasicParsing
        Write-Host("   --Starting installer...")
        Start-Process -FilePath $dockerInstaller
    } else {
        Write-Host("   --Found an existing install.")
    }
...
if ($rebootRequired) {
    ...
    if ($distro.AppxName.Length -gt 1) {
        ...
        Write-Host(">> Create a UNIX username and password.")
        ...
        Write-Host("-----------------------------------------")
        Write-Host("`n-----------------------------------------")
        Write-Host("> Your PC is ready to run Docker!")
        Write-Host("-----------------------------------------")
        Write-Host("> Maintained by CryptoKasm")
        Write-Host("   --Github - https://github.com/CryptoKasm")
        Write-Host("   --Support - https://discord.gg/k6z2GS4yh2")
        Write-Host("-----------------------------------------")
    } else {
        $wslselect = ""
